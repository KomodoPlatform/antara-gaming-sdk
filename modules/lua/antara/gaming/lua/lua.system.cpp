...
void scripting_system::update() noexcept {
  entity_registry_.view<lua::component_script>().each(
      [this](auto entity_id, auto &&comp) {
...
scripting_system::scripting_system(entt::registry &entity_registry,
    ...
    : system(entity_registry), directory_path_(std::move(script_directory)),
      systems_directory_path_(std::move(systems_directory)),
  ...
  if (!res)
    std::abort();
  sol::table table =
      lua_state_->create_table_with("version", gaming::version());
  ...
  (*this->lua_state_)["entt"] = lua_state_->create_table_with(
      "entity_registry", std::ref(this->entity_registry_), "dispatcher",
      std::ref(this->dispatcher_));
}
...
void scripting_system::register_entity_registry() {
  ...
  (*this->lua_state_)["entity_registry"]["destroy"] =
      [](entt::registry &self, entt::registry::entity_type entity) {
        self.destroy(entity);
      };

  (*this->lua_state_)["entity_registry"]["valid"] =
      [](entt::registry &self, entt::registry::entity_type entity) {
        return self.valid(entity);
      };

  (*this->lua_state_)["entity_registry"]["for_each_runtime"] =
      [](entt::registry &self, std::vector<ENTT_ID_TYPE> components,
         sol::function functor) {
        return self.runtime_view(std::cbegin(components), std::cend(components))
            .each([func = std::move(functor)](auto entity) { func(entity); });
      };
}
...
bool scripting_system::load_script_from_entities() noexcept {
  bool res = true;
  entity_registry_.view<lua::component_script>().each(
      [this, &res](auto entity_id, auto &&comp) {
...
bool scripting_system::load_scripted_system(
    const std::string &script_name) noexcept {
  ...
  case ecs::pre_update:
    this->dispatcher_.trigger<ecs::event::add_base_system>(
        std::make_unique<details::lua_pre_scripted_system>(
            entity_registry_, table_name, this->lua_state_));
    break;
  case ecs::logic_update:
    this->dispatcher_.trigger<ecs::event::add_base_system>(
        std::make_unique<details::lua_logic_scripted_system>(
            entity_registry_, table_name, this->lua_state_));
    break;
  case ecs::post_update:
    this->dispatcher_.trigger<ecs::event::add_base_system>(
        std::make_unique<details::lua_post_scripted_system>(
            entity_registry_, table_name, this->lua_state_));
    break;
  case ecs::size:
    break;
...
bool scripting_system::load_script(const std::string &file_name,
                                   const fs::path &script_directory) noexcept {
  try {
    this->lua_state_->script_file((script_directory / file_name).string());
  } catch (const std::exception &error) {
    VLOG_F(loguru::Verbosity_ERROR,
           "error when loading script: {} err: {} script_directory: {}",
