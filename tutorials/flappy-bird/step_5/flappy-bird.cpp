...
namespace {
    ...
    void create_player(entt::registry &registry, sfml::resources_manager &resource_mgr) {
        //! Retrieve constants
        const auto [canvas_width, canvas_height] = registry.ctx<graphics::canvas_2d>().canvas.size;
        const auto constants = registry.ctx<flappy_bird_constants>();
        ...
        //! Entity components
        auto &sprite = registry.assign<sfml::sprite>(entity, sf::Sprite(*texture)).drawable;
        sprite.setOrigin(sprite.getLocalBounds().width * 0.5f, sprite.getLocalBounds().height * 0.5f);
        sprite.setPosition(canvas_width * 0.2f, canvas_height * 0.2f);
...
public:
    game_scene(entt::registry &entity_registry) noexcept : base_scene(entity_registry) {
        ...
        create_player(entity_registry_, resource_mgr);
    }
