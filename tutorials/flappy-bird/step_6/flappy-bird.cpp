...
public:
    ...
    void update() noexcept final {
        ...
        // If Space is tapped, add jump force to the movement speed
        bool jump_key_pressed = input::is_key_pressed(input::space);
        if(jump_key_pressed && !jump_key_pressed_last_tick) {
            movement_speed.set_y(-constants.jump_force);
        }
