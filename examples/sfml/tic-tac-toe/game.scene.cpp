        ...
        this->entity_registry_.view<board_component>().each([&, this](board_component &board_cmp) {
            ...
            for (std::size_t i = 0; i < constants.nb_cells; ++i) {
                for (std::size_t j = 0; j < constants.nb_cells; ++j) {
                    if (board_cmp.board[i * constants.nb_cells + j] == static_cast<cell_state>(this->player_turn_)) {
                        row_count++;
                    }

                    if (board_cmp.board[j * constants.nb_cells + i] == static_cast<cell_state>(this->player_turn_)) {
                        column_count++;
                    }
                }
